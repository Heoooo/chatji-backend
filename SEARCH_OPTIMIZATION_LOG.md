...
*   **Metric**:
    *   **검색 커버리지 (Keyword Diversity)**: 단일 키워드(100개) → 다중 키워드(최대 300개) 확장 운영.
    *   **유효 상품 확보율**: 기존 대비 **약 200~300% 증가** (동의어 기반 상품 누락 차단).
    *   **검색 정확도**: 단순 일치 검색에서 **의미적 유사도 매칭**으로 진화.

### 2. 성능 유지 전략
*   **Action**: 총 API 호출 횟수를 기존(3회)과 동일하게 유지하되, 한 키워드의 깊이(Depth) 대신 검색어의 너비(Breadth)를 확장하여 성능 저하 없이 품질만 개선.
